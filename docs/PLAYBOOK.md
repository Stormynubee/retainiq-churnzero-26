...
| Action | Who | Why |
|---|---|---|
| **Call first** | High `churn_probability` + **persuadable** uplift segment | Offer actually helps (~4 in training) |
| **Standard retention** | High churn score, not sleeping-dog | FN costs ₹40,000 each |
| **Do not push offers** | **Sleeping-dog** segment (141 in training) | CATE ≈ −0.27 — outreach may increase churn |
...
At t ≈ 0.001 on OOF: **₹62,500** total cost vs **₹200,500** at t = 0.5 — **~69% savings** with same model.

## 4. Fairness gate

Before rolling out campaigns by segment, check `data/processed/fairness_summary.csv`:

- Gender demographic parity gap: **2.7%**
- Region demographic parity gap: **1.3%**
